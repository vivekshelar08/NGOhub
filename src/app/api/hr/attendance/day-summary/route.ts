...
import { todayDateOnly, localDateKey } from "@/lib/hr-utils";
...
export async function GET() {
  ...
  try {
    ...
    const completed = tasks.filter(
      (t) => t.status === "completed" && t.completedAt?.startsWith(todayKey)
    ).length;
    ...
    return NextResponse.json({
      date: todayKey,
      attendance,
      tasks: { assigned: 0, active: 0, completed: 0, total: 0 },
      fieldVisits: 0,
      onLeave: attendance?.status === "LEAVE",
      schemaPending: true,
    });
